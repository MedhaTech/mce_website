...
Class Admin_model extends CI_Model
{
  ...
  function facultyLogin($username, $password)
 {
  ...
  function naacfiles($id){
    $this->db->select('title, `order`,`file_names`');
    $this->db->from('naac_files');
    $this->db->where('naac_id', $id);
    $this->db->order_by("CAST(SUBSTRING_INDEX(`order`, '.', 1) AS UNSIGNED)");
    $this->db->order_by("CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(`order`, '.', 2), '.', -1) AS UNSIGNED)");
    $this->db->order_by("CAST(SUBSTRING_INDEX(`order`, '.', -1) AS UNSIGNED)");
    return $this->db->get();
...
function getDetailsbyfield2sort($id1, $value1, $id2, $value2, $tableName,$sortField, $srotType){
  $this->db->where($id1, $value1);
  $this->db->where($id2, $value2);
  $this->db->order_by($sortField, $srotType);
  return $this->db->get($tableName);
...
function getDetailsbyfield2sort_new($id1, $value1, $id2, $value2, $tableName,$sortField, $srotType){
  $this->db->where($id1, $value1);
  $this->db->where($id2, $value2);
  $this->db->order_by($sortField, $srotType);
  $this->db->limit(15); 
