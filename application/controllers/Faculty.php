...
class Faculty extends CI_Controller
{
	...
	function researchPatents()
	{
		if ($this->session->userdata('logged_in')) {
			...
			$Details = $this->admin_model->getDetailsbyfield($data['id'], 'faculty_id', 'research_patents')->result();
			...
			if ($Details != null) {
				...
				foreach ($Details as $Details1) {
					$btn = '<div class="btn-group">
				' . anchor('faculty/viewResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-eye"></i>', 'class="btn btn-sm btn-primary" data-toggle="tooltip" title="View"') . '	
				' . anchor('faculty/editResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-pencil-alt"></i>', 'class="btn btn-sm btn-success" data-toggle="tooltip" title="Edit"') . '
				' . anchor('faculty/deleteResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-times"></i>', 'class="btn btn-sm btn-danger" data-toggle="tooltip" title="Remove"') . '
				</div>';
			...
			$this->faculty_template->show('faculty/researchPatents', $data);
		} else {
			redirect('faculty', 'refresh');
		}
	}
	...
	function addResearchPatents()
	{
		if ($this->session->userdata('logged_in')) {
			...
			if ($this->form_validation->run() === FALSE) {
				$data['action'] = 'faculty/addResearchPatents/';
				$data['btn_name'] = 'Add';
				...
				$this->faculty_template->show('faculty/addResearchPatents', $data);
			} else {
				...
				redirect('faculty/researchPatents', 'refresh');
			}
		} else {
			redirect('faculty', 'refresh');
		}
	...
	function editResearchPatents($id)
	{
		if ($this->session->userdata('logged_in')) {
			...
			if ($this->form_validation->run() === FALSE) {
				$data['action'] = 'faculty/editResearchPatents/' . $id;
				$data['btn_name'] = 'Edit';
	...
	function viewResearchPatents($id)
	{
		if ($this->session->userdata('logged_in')) {
			...
			$this->faculty_template->show('faculty/viewResearchPatents', $data);
		} else {
			redirect('faculty', 'refresh');
		}
	}
