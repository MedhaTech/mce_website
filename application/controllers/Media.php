...
class Media extends CI_Controller
{
    ...
    public function createFolder($parent = '')
    {
        if ($this->session->userdata('logged_in')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                if ($parent == '') {
                    $this->aws_sdk->createFolder($folderName, 'mcehassan-ac-in');
                } else {
                    $this->aws_sdk->createFolderInFolder('mcehassan-ac-in', $parent, $folderName);
                }
    ...
    public function uploadFile()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $config['allowed_types'] = 'gif|jpg|png|pdf|xlsx|csv|mp4';
                ...
                if (!$this->upload->do_upload('userfile')) {
                    ...
                    $result =  $this->aws_sdk->uploadFile($filePath, 'mcehassan-ac-in', $folderName);
    ...
    public function viewFolderContents()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $bucketName = 'mcehassan-ac-in'; // Replace with your bucket name
