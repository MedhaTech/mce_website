...
class Dept extends CI_Controller
{
	...
	public $designationType1 = array('4' => 'Dean','5' => 'Associate Dean');
		...
		if ($this->session->userdata('logged_in')) {
			...
			if ($faculty != null) {
				...
				foreach ($faculty as $faculty1) {
					...
					if($data['id']=='27')
					{
						$designa=$this->designationType1[$faculty1->designation_id] . $HOD . $Principal . $VicePrincipal;
					}
					else
					{
						$designa=$this->designationType[$faculty1->designation_id] . $HOD . $Principal . $VicePrincipal;
					}
	...
	function addFaculty()
	{
		if ($this->session->userdata('logged_in')) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				if($data['id']=='27')
					{
						$data['designationType'] = array(' ' => "Select") + $this->designationType1;
					}
					else
					{
		...
		if ($this->session->userdata('logged_in')) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				$insertData = array(
					...
					'dept_id' => $data['id'],
					'new_label' => $this->input->post('new_label') ? 1 : 0,
					'status' => '1'
				);
...
function addSeats()
	...
	function researchPatents()
	{
		if ($this->session->userdata('logged_in')) {
			...
			$data['department_name'] = $session_data['department_name'];
			$data['short_name'] = $session_data['short_name'];
			$data['pageTitle'] = "Research Patents";
			$data['activeMenu'] = "research";

			$Details = $this->admin_model->getDetailsbyfield($data['id'], 'dept_id', 'research_patents')->result();
			...
			if ($Details != null) {
				...
				foreach ($Details as $Details1) {
					$btn = '<div class="btn-group">
				' . anchor('dept/viewResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-eye"></i>', 'class="btn btn-sm btn-primary" data-toggle="tooltip" title="View"') . '	
				' . anchor('dept/editResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-pencil-alt"></i>', 'class="btn btn-sm btn-success" data-toggle="tooltip" title="Edit"') . '
				' . anchor('dept/deleteResearchPatents/' . $Details1->id, '<i class="fa fa-fw fa-times"></i>', 'class="btn btn-sm btn-danger" data-toggle="tooltip" title="Remove"') . '
				</div>';
			...
			$this->dept_template->show('dept/researchPatents', $data);
		} else {
			redirect('dept', 'refresh');
		}
	}

	function addResearchPatents()
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$data['department_name'] = $session_data['department_name'];
			$data['short_name'] = $session_data['short_name'];
			$data['pageTitle'] = "Add New Research Patents";
			$data['activeMenu'] = "research";

			$this->form_validation->set_rules('title', 'Title', 'required');

			if ($this->form_validation->run() === FALSE) {
				$data['action'] = 'dept/addResearchPatents/';
				$data['btn_name'] = 'Add';
				...
				$this->dept_template->show('dept/addResearchPatents', $data);
			} else {
				...
				$res = $this->admin_model->insertDetails('research_patents', $insertDetails);
				if ($res) {
					$this->session->set_flashdata('message', 'Research Collaboration Details inserted successfully...!');
					$this->session->set_flashdata('status', 'alert-success');
				} else {
				...
				redirect('dept/researchPatents', 'refresh');
			}
		} else {
			redirect('dept', 'refresh');
		}
	...
	function editResearchPatents($id)
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$data['department_name'] = $session_data['department_name'];
			$data['short_name'] = $session_data['short_name'];
			$data['pageTitle'] = "Edit Research Patents";
			$data['activeMenu'] = "research";
			$data['eid'] = $id;

			$this->form_validation->set_rules('title', 'Title', 'required');
			$Details = $this->admin_model->getDetails('research_patents', $id)->row();

			if ($this->form_validation->run() === FALSE) {
				$data['action'] = 'dept/editResearchPatents/' . $id;
				$data['btn_name'] = 'Edit';
				...
				$res = $this->admin_model->updateDetails($id, $updateDetails, 'research_patents');
				if ($res) {
					$this->session->set_flashdata('message', 'Research Publication Details updated successfully...!');
					$this->session->set_flashdata('status', 'alert-success');
				} else {
	...
	function viewResearchPatents($id)
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$data['department_name'] = $session_data['department_name'];
			$data['short_name'] = $session_data['short_name'];
			$data['pageTitle'] = "View Research Patents";
			$data['activeMenu'] = "research";
			$data['eid'] = $id;

			$data['Details'] = $this->admin_model->getDetails('research_patents', $id)->row();
			$this->dept_template->show('dept/viewResearchPatents', $data);
		} else {
			redirect('dept', 'refresh');
		}
	}

	function deleteResearchPatentsFile($id, $filenameID)
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$data['department_name'] = $session_data['department_name'];
			$data['short_name'] = $session_data['short_name'];
			$data['pageTitle'] = "Remove Research Patents";
			$data['activeMenu'] = "research";
			$data['eid'] = $id;
			...
			$result = $this->admin_model->updateDetails($id, $updateDetails, 'research_patents');
	...
	function deleteResearchPatents($id)
	{
		if ($this->session->userdata('logged_in')) {
			$session_data = $this->session->userdata('logged_in');
			$data['id'] = $session_data['id'];
			...
			$this->admin_model->delDetails('research_patents', $id);

			$this->session->set_flashdata('message', 'Research Patent Details deleted successfully...!');
			$this->session->set_flashdata('status', 'alert-success');
