            ...
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col mr-auto">
                        <h4 class="card-title m-0">Folders : <?php echo $currentFolderPath; ?></h4>
                    </div>
                ...
                <div id="main-folders" class="d-flex align-items-stretch flex-wrap">
                        ...
                        ?>
                         <?php foreach ($objects['folders'] as $folder) : ?>
                            <div class="d-inline-flex">
                                <a href="<?php echo base_url('media/viewFolderContents') . '?folder=' . urlencode($folder['Prefix']); ?>">
                                <button class="folder-container">
                                    <div class="folder-icon">
                                        <i class="fa fa-folder folder-icon-color"></i>
                                    </div>
                                    <div class="folder-name"><?php echo end(explode('/', trim($folder['Prefix'], '/'))); ?></div>
                                    </button>
                                </a>
                            </div>
                        <?php endforeach; ?>
                    ...
                    <?php if (!empty($objects['files'] )) : 
                       ...
                       <?php foreach ($objects['files'] as $file) :
                        if($file['Size']!=0) : ?>
                            <div class="d-inline-flex">
                                <button class="folder-container fold"  id="https://mcehassan-ac-in.s3.ap-south-1.amazonaws.com/<?php echo $file['Key'];  ?>">
                                    <div class="folder-icon">
                                        <i class="fa fa-file file-icon-color"></i>
                                    </div>
                                    <div class="folder-name" ><?php echo end(explode('/', trim($file['Key'], '/')));  ?></div>
                                </button>
                            </div>
